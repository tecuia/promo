...
(function() {
  ...
  function disableScroll() {
    ...
    window._originalScrollBehavior = {
  ...
  function enableScroll() {
    const orig = window._originalScrollBehavior;
    if (!orig) return;
    
    const scrollY = orig.scrollY || Math.abs(parseInt(document.body.style.top || '0'));
    ...
    window._originalScrollBehavior = null;
    window.scrollTo(0, scrollY);
  }

  // ============================================
  // SCROLL BLOCKER (shared)
  // ============================================
  ...
  function setupScrollBlocker(scrollLimit) {
  ...
  // ============================================
  // DETECT MECHANIC TYPE
  // ============================================
  
  function getMechanicType() {
    if (document.querySelector('.quiz-window')) return 'quiz';
    if (document.querySelector('.tinder-window')) return 'tinder';
    if (document.querySelector('.randomizer-window')) return 'randomizer';
    return null;
  }
  ...
  function initTinderMechanic() {
    ...
    async function loadData() {
      try {
        ...
        cardData = [
          { "name": "Дорогие товары(премиум)", "image": "imgs/1_1.png", "correct": true, "type": "type-1", "description": "Большинство1 компаний по-прежнему оценивают эффективность маркетинга через привычные метрики: стоимость клика, охват, количество переходов. Эти показатели важны, но они описывают только начало " },
          { "name": "Новинки и эксперементы", "image": "imgs/2_1.png", "correct": true, "type": "type-1", "description": "Большинство2 компаний по-прежнему оценивают эффективность маркетинга через привычные метрики: стоимость клика, охват, количество переходов. Эти показатели важны, но они описывают только начало " },
          { "name": "Товары по акции или скидке", "image": "imgs/3_1.png", "correct": true, "type": "type-1", "description": "Большинство3 компаний по-прежнему оценивают эффективность маркетинга через привычные метрики: стоимость клика, охват, количество переходов. Эти показатели важны, но они описывают только начало " }
        ];
      }
      initScrollAnimation();
      loadCards();
    ...
    function updateDescriptionText(cardData) {
      ...
      gsap.to(cardDescriptionText, {
        ...
        duration: 0.3,
        onComplete: () => {
          cardDescriptionText.textContent = cardData?.description || 'Большинство компаний по-прежнему оценивают эффективность маркетинга через привычные метрики: стоимость клика, охват, количество переходов. Эти показатели важны, но они описывают только начало';
  ...
  function initRandomizerMechanic() {
    const CONTENT_MAP = {
      park: {
    ...
    function collapseWindowForCompletion() {
      ...
      gsap.set(wrapper, { opacity: 0, y: 30 });
    ...
    window.resetMechanic = function() {
      ...
      if (randomizerFiltersWrapper) {
        gsap.set(randomizerFiltersWrapper, { opacity: 1, y: 0, clearProps: "all" });
      }
      
      if (randomizerSubmitBtn) {
        gsap.set(randomizerSubmitBtn, { opacity: 1, clearProps: "all" });
      }
      ...
      if (randomizerResult) {
        ...
        gsap.set(randomizerResult, { opacity: 0, y: 0, filter: 'blur(0px)', clearProps: "all" });
      }
  ...
  if (mechanicType === 'quiz') {
    initQuizMechanic();
  } else if (mechanicType === 'tinder') {
    initTinderMechanic();
  } else if (mechanicType === 'randomizer') {
    initRandomizerMechanic();
  }
